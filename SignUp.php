...
?>
    ...
    <?php
        ...
        if (empty($email) || empty($id) ||empty($name)|| empty($password) || empty($conPassword) || empty($phone)||!preg_match("/^([a-z0-9\+_\-]+)(\.[a-z0-9\+_\-]+)*@([a-z0-9\-]+\.)+[a-z]{2,6}$/ix", $email)||!ctype_alpha(str_replace(' ', '', $name))||strlen($password)<=5||((!preg_match('/[^A-Za-z]/', $password))||(is_numeric($password) == 1))||$conPassword !== $pass||is_numeric($phone) == 0||(strlen($phone)>10) || (strlen($phone)<10)){
        ...
        //If login sucessful
                        ...
                        echo "<div id='sign-up-form'>";
                        echo "<img src='images/user.png' alt='user icon' width='70' height='70'><p>&nbsp;</p>";
                        echo "<h1>You Have Been Succefully Registered !</h1>"; 
                        echo "</div>"; // end the css of the sign-up-form
            }
